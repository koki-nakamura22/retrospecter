...
| [session-retrospecter](./session-retrospecter) | Claude Code セッションログ (`~/.claude/projects/*.jsonl`) | Released |
| retrospect-core | 共通ライブラリ: `Knowledge` モデル / classifier ラッパ / cache 基底 / renderer 基底 | Phase 3 (2 つ目アプリの共通面が安定してから抽出) |
...
retrospecter/
├── README.md
├── repo-retrospecter/        ← 1 つ目のアプリ: GitHub 履歴 → 振り返り + AI Knowledge
│   ├── README.md             ← 詳細な使い方 / install / 例
│   ├── pyproject.toml
│   ├── src/repo_retrospecter/
│   └── tests/
└── session-retrospecter/     ← 2 つ目のアプリ: Claude Code セッションログ → 振り返り + AI Knowledge
    ├── README.md
    ├── pyproject.toml
...
`retrospect-core` パッケージを抽出する場合 (下記ポリシー参照) はアプリと同階層に配置します。
...
1. **Phase 1 (完了)** — umbrella 配下にアプリ 1 つ。core なし。
2. **Phase 2 (完了)** — `repo-retrospecter` を単独で出荷。依然 core なし。
3. **Phase 3 (現在)** — `session-retrospecter` が `repo-retrospecter` と並んで登場。本当に共通する部分を 2 つの実コードから観測するため、しばらくは敢えて重複を許容して運用する。"rule of three" — 推測ではなく、2 箇所で実際に重複が観測され、共通面が安定してから初めて `retrospect-core` を uv workspace member として抽出する。
...
- [session-retrospecter — README](./session-retrospecter/README.ja.md)
